...
public String insertDatabase() throws Exception {		
	  		...
	  		for (WebElement ele : eleList)           // To insert data into the database for the first time! 
...
{ 
	List<Trailer> allList=trailerService.findByFilter("select1",null);
	for(int i=0;i<100;i++){
		Trailer trailer=allList.get(i);
	 	trailerService.update("updateview", trailer);  //to First Time Set the view Count of each movie to be zero!
	}
	System.out.println("Each View Count Set");
	return "new";
